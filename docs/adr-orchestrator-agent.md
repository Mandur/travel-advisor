...
- Should we adopt an agent-per-domain or task-graph-style orchestrator?
- What is the minimal viable authorization model for the prototype?
- How should we structure the adapter interfaces to allow future decomposition?
- What telemetry and observability should be built for the prototype (even if minimal)?

## 9. Next Steps

- Define the agent contract schema (request envelope, routing metadata, auth context).
- Implement domain classifier (rules + embedding prototype).
- Build two adapters:
	- Meeting Broker (Mockzilla)
	- Travel Advisor QA Endpoint
- Create a Teams/Copilot-friendly response formatter.
- Implement basic role allowance filter.